...
#extract 100 sample from cifar100 and save it to csv file
def extractFromCifar100():
    ...
    # 랜덤하게 100개의 인덱스 생성
    indices = np.random.choice(len(test_data), 100, replace=False)

    # 선택된 샘플과 레이블 추출
    sampled_data = test_data[indices]
    sampled_labels = test_labels[indices]

    # 이미지 데이터를 CSV 파일로 저장하기 위해 1D 배열로 변환
    sampled_data_reshaped = sampled_data.reshape(sampled_data.shape[0], -1)
    df = pd.DataFrame(sampled_data_reshaped)
    df.insert(0, 'label', sampled_labels)  # Add 'label' column as the first column
    # CSV 파일로 저장
    csv_file = os.path.join(data_folder, 'cifar100_samples.csv')
    df.to_csv(csv_file, index=False, header=False)  # Remove column names
